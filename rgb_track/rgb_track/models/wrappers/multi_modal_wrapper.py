...
class MultiModalWrapper(SimpleClassifierWrapper):
    ...
    def _init_modules(self, wrapper_config):
        ...
        self.input_image_modalities = wrapper_config.input_image_modalities
        for modal_key in self.input_modalities:
            if modal_key == 'optical_flow':
        ...
        self.image_backbone, feature_size = get_backbone(wrapper_config.backbone,
                                                  pretrained=wrapper_config.pretrained,
                                                  get_feature_size=True,in_channels = 6)
    ...
    def forward(self, x):
        ...
        for k, v in x.items():
            if k not in ['data', 'target'] + self.input_modalities + self.input_image_modalities:
                output_dict[k] = v
